...
switch ($act) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $newfile = basename($file); 
             ...
             if(in_array($ext,$allow_ext)){
               ...
               if($size<=$size_allow){
               $upload=move_uploaded_file($_FILES['filename']['tmp_name'],'View/assets/upload/'.$newfile);
               }
             }
             else {
                include_once "./View/qldoan/add_doan.php";
    ...
    case "update_action":
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if ($check !== false) {
                ...
                echo '<script>alert("Cập nhật đồ án không thành công");</script>';
                include_once "./View/qldoan/edit_doan.php";
            }
        }
        break;
    ...
    case "update_points_action":
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if ($check !== false) {
                echo '<script>alert("Cập nhật điểm thành công");</script>';
                echo '<meta http-equiv=refresh content="0;url=index.php?action=doan"/>';
            } else {
